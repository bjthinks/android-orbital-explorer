...
void main() {
    ...
    vec3 lb = texelFetch(data, leftBottom, 0).xyz;
    vec3 lt = texelFetch(data, ivec2(leftBottom.x, rightTop.y), 0).xyz;
    vec3 rb = texelFetch(data, ivec2(rightTop.x, leftBottom.y), 0).xyz;
    vec3 rt = texelFetch(data, rightTop, 0).xyz;

    vec2 interp = fract(texCoord);
    color = mix(mix(lb, rb, interp.x), mix(lt, rt, interp.x), interp.y);
}
